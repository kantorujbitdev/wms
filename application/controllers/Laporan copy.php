...
class Laporan extends MY_Controller
{
    ...
    public function keluar_masuk()
    {
        $this->check_permission('laporan', 'view');
        // Set title
        $this->data['title'] = 'Laporan Keluar / Masuk';
        $this->data['active_menu'] = 'laporan';
        $this->data['active_submenu'] = 'laporan_keluar_masuk';
        ...
        $filter_date_from = $this->input->get('date_from') ?: null;
        $filter_date_to = $this->input->get('date_to') ?: null;
        $filter_transaksi_type = $this->input->get('transaksi_type') ?: null;
        $filter_source_type = $this->input->get('source_type') ?: null;
        ...
        $is_filtered = !empty($filter_date_from) || !empty($filter_date_to) ||
            !empty($filter_transaksi_type) || !empty($filter_source_type);

        // Jika ada filter, ambil data dari API
        if ($is_filtered) {
            // Prepare filter data for API
            $params = [
                'date_from' => $filter_date_from,
                'date_to' => $filter_date_to,
                'transaksi_type' => $filter_transaksi_type,
                'source_type' => $filter_source_type
            ];

            // Get report from API berdasarkan jenis transaksi
            if ($filter_transaksi_type == 'in' || empty($filter_transaksi_type)) {
                $response_in = $this->Api_model->get_laporan_masuk($params);
                $in_report = ($response_in['success'] && isset($response_in['data'])) ? $response_in['data'] : [];
            }

            if ($filter_transaksi_type == 'out' || empty($filter_transaksi_type)) {
                $response_out = $this->Api_model->get_laporan_keluar($params);
                $out_report = ($response_out['success'] && isset($response_out['data'])) ? $response_out['data'] : [];
            }
        }

        // Pass data to view
        $this->data['in_report'] = $in_report;
        $this->data['out_report'] = $out_report;
        $this->data['is_filtered'] = $is_filtered;
        $this->data['filter_date_from'] = $filter_date_from;
        $this->data['filter_date_to'] = $filter_date_to;
        $this->data['filter_transaksi_type'] = $filter_transaksi_type;
        $this->data['filter_source_type'] = $filter_source_type;
        $this->data['user_role'] = $user_role;

        // Render view
        $this->render_view('pages/laporan/keluar_masuk');
    }
