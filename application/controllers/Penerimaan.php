...
class Penerimaan extends MY_Controller
{
    ...
    public function dari_supplier()
    {
        ...
        // Get penerimaan data from API dengan filter from_status = 2 (Supplier)
    ...
    public function dari_pengguna()
    {
        ...
        // Get penerimaan data from API dengan filter from_status = 2 (pengguna)
        $response = $this->Api_model->get_penerimaan($data_login);
        $this->data['penerimaan_list'] = $response['success'] ? $response['data'] : [];
    ...
    public function antar_gudang()
    {
        ...
        // Get penerimaan data from API dengan filter from_status = 3 (Antar Gudang)
        $response = $this->Api_model->get_penerimaan($data_login);
        $this->data['penerimaan_list'] = $response['success'] ? $response['data'] : [];
    ...
    public function add_supplier()
    {
        ...
        // SESUAIKAN: get_barang() bukan get_product_all()
        ...
        // Set from_status untuk supplier = 2
    ...
    public function add_pengguna()
    {
        ...
        $pengguna_response = $this->Api_model->get_customer($data_login);
        $this->data['penggunas'] = $pengguna_response['success'] ? $pengguna_response['data'] : [];

        // SESUAIKAN: get_barang() bukan get_product_all()
        $products_response = $this->Api_model->get_barang($data_login);
        $this->data['products'] = $products_response['success'] ? $products_response['data'] : [];

        // Set from_status untuk pengguna = 1
    ...
    public function add_antar_gudang()
    {
        ...
        // Untuk antar gudang, tidak perlu supplier
        $this->data['suppliers'] = [];
        ...
        // Set from_status untuk antar gudang = 3
    ...
    public function create()
    {
        if ($_POST) {
            ...
            if ($response['success']) {
                ...
                if ($post_data['from_status'] == '3') {
                    redirect('penerimaan/antar_gudang');
                } else {
                    redirect('penerimaan/from_supplier');
                }
            } else {
                $this->session->set_flashdata('error', $response['message'] ?? 'Gagal menambahkan penerimaan barang');

                // Redirect back based on from_status
                if ($this->input->post('from_status') == '3') {
                    redirect('penerimaan/form_antar_gudang');
                } else {
                    redirect('penerimaan/form_supplier');
                }
            }
        }
    }
    ...
    public function detail($id)
    {
        ...
        // Get penerimaan detail from API
    ...
    public function delete($id)
    {
        ...
        $response = $this->Api_model->update_penerimaan($data_login); // atau delete_penerimaan jika ada
