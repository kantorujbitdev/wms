<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Form Transfer Stok</h6>
            <a href="<?php echo site_url('transaksi'); ?>"
                class="d-none d-sm-inline-block btn btn-sm btn-secondary shadow-sm">
        ...
        <div class="card-body">
            <?php echo form_open('transaksi/save_transfer'); ?>
...
<script>
    $(document).ready(function () {
        ...
        $('#from_warehouse_id').change(function () {
            var fromId = $(this).val();
            $('#to_warehouse_id option').prop('disabled', false);
            $('#to_warehouse_id option[value="' + fromId + '"]').prop('disabled', true);
        });

        $('#to_warehouse_id').change(function () {
            var toId = $(this).val();
            $('#from_warehouse_id option').prop('disabled', false);
            $('#from_warehouse_id option[value="' + toId + '"]').prop('disabled', true);
        });
    });
</script>